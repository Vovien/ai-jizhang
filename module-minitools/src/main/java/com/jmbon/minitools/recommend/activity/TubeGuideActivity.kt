...
import com.jmbon.minitools.recommend.router.RecommendRouter
...
class TubeGuideActivity : ViewModelActivity<ChooseViewModel, ActivityTubeGuideLayoutBinding>() {
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        binding.tvLeft.onClick {
            ...
            BuryHelper.addEvent(
                BuryPointInfo(
                    event_id = ClickEventEnum.EVENT_CLICK_REGISTER_TRANSFORM_BOTTOM_BUTTON.value,
                    params = mutableMapOf(
                        Pair("image_id", "$guidePictureId"),
                        Pair("position", "left")
                    )
                )
            )
