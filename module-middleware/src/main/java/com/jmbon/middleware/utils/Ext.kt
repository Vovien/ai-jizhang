...
private val tagBgColor by lazy {
    R.color.color_FFF5F0.toColorInt()
}

/**
 * 标签的文本颜色
 */
private val tagTextColor by lazy {
    R.color.color_FF5F0A.toColorInt()
}

fun FlexboxLayout.initTagLayout(tagList: List<String>?) {
    removeAllViews()
    ...
    isVisible = true
    setDividerDrawable(getDivider(Color.TRANSPARENT, 4.dp, 4.dp))
    tagList.forEach {
        TextView(context).apply {
            ...
            setBackground(backgroundColor = tagBgColor, radius = 5.dp)
            addView(this)
        }
    }
}
...
fun RecyclerView.init(
    adapter: BaseQuickAdapter<*, *>?,
    layoutManager: RecyclerView.LayoutManager = getLayoutManager()
