...
open class CommonViewModel : BaseViewModel() {
    ...
    fun getCommonBanner() {
        viewModelScope.launch {
            launchWithFlow({
                ...
                commonBannerFlow.value = null
            }).next {
                commonBannerFlow.value = this
                GsonUtils.toJson(this).saveToMMKV(MMKVKey.COMMON_BANNER)
            }
        }
    }
    ...
    companion object {
         ...
         * 公共bannerFlow
         */
        val commonBannerFlow = MutableStateFlow<CommonBannerBean?>(null)
            get() {
                if (field.value == null) {
                    try {
                        field.value = GsonUtils.fromJson(
                            MMKVKey.COMMON_BANNER.getString(),
                            CommonBannerBean::class.java
                        )
                    } catch (e: Exception) {
        ...
        fun activeDevice(oaid: String?) {
            GlobalScope.launch {
                ...
                launchWithFlow({
                    API.uploadOAID(sysVersion, deviceModel, oaid)
                    API.activeApp(oaid)
                }).next {}
            }
        }
